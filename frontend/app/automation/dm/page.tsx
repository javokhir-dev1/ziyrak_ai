...
export default function DmFullPage() {
  ...
  useEffect(() => {
    ...
    getDmMessages().then(d => { setMessages(d.messages); setCurrentIndex(d.currentIndex); });
  }, []);

  const showAlert = (type: 'success' | 'error', msg: string) => { setAlert({ type, msg }); setTimeout(() => setAlert(null), 3000); };

  const saveSettings = async () => {
    setSaving(true);
    try { await updateSettings({ dmAutoReplyEnabled }); showAlert('success', 'Sozlamalar saqlandi'); }
    catch { showAlert('error', 'Xato'); } finally { setSaving(false); }
  };

  const saveDmMessages = async () => {
    const filtered = messages.filter(m => m.trim());
    if (!filtered.length) return showAlert('error', 'Kamida 1 ta xabar kerak');
    setSaving(true);
    try { await updateDmMessages(filtered); setMessages(filtered); setCurrentIndex(0); showAlert('success', 'DM xabarlar saqlandi'); }
    catch { showAlert('error', 'Xato'); } finally { setSaving(false); }
  };
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <header className="bg-surface/80 backdrop-blur-xl border-b border-outline-variant/30 px-8 py-5 flex items-center gap-4 sticky top-0 z-10">
        ...
        <div>
          <h2 className="text-[20px] font-semibold text-slate-900 leading-tight">DM avto javob</h2>
          <p className="text-[13px] text-slate-400">Kiruvchi xabarlarga avtomatik javob</p>
        </div>
      </header>
      ...
      <div className="flex-1 overflow-y-auto p-8">
        <div className="max-w-2xl mx-auto space-y-5">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-6">
            <Toggle checked={dmAutoReplyEnabled} onChange={setDmAutoReplyEnabled} label="DM Avtojavobni yoqish" description="Kimdir DM yozganida aylanadigan xabarlardan javob yuboradi" />
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-6">
            ...
            <div className="space-y-2.5">
              {messages.map((msg, i) => (
                <div key={i} className={`flex gap-2.5 items-start border-[1.5px] rounded-xl px-3 py-2.5 transition-colors ${
                  ...
                  <button onClick={() => removeMsg(i)} className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors flex-shrink-0">
                    <Trash2 size={14} />
                  </button>
                </div>
              ))}
            ...
            <div className="flex items-center justify-between mt-4">
              <button onClick={addMsg} className="flex items-center gap-1.5 text-sm px-4 py-2 border-[1.5px] border-gray-200 text-gray-600 rounded-xl hover:bg-gray-50 transition-colors">
